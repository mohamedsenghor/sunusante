...
export const Patient = () => {
  ...
  const patientList = useAppSelector(state => state.sunusantegatewayservice.patient.entities);
  const loading = useAppSelector(state => state.sunusantegatewayservice.patient.loading);
  const totalItems = useAppSelector(state => state.sunusantegatewayservice.patient.totalItems);
  ...
  return (
    <div>
      <h2 id="patient-heading" data-cy="PatientHeading">
        <Translate contentKey="sunusanteGatewayServiceApp.sunusantePatientServicePatient.home.title">Patients</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="sunusanteGatewayServiceApp.sunusantePatientServicePatient.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/patient/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="sunusanteGatewayServiceApp.sunusantePatientServicePatient.home.createLabel">
              Create new Patient
            </Translate>
          </Link>
        </div>
        ...
        {patientList && patientList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="sunusanteGatewayServiceApp.sunusantePatientServicePatient.id">ID</Translate>{' '}
                ...
                <th className="hand" onClick={sort('idValue')}>
                  ...
                  <FontAwesomeIcon icon={getSortIconByFieldName('idValue')} />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {patientList.map((patient, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`sunusanteGatewayServiceApp.IdentifierType.${patient.idType}`} />
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="sunusanteGatewayServiceApp.sunusantePatientServicePatient.home.notFound">No Patients found</Translate>
            </div>
          )
        )}
      </div>
