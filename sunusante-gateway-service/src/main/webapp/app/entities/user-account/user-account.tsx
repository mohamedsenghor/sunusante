...
export const UserAccount = () => {
  ...
  const userAccountList = useAppSelector(state => state.sunusantegatewayservice.userAccount.entities);
  const loading = useAppSelector(state => state.sunusantegatewayservice.userAccount.loading);
  const totalItems = useAppSelector(state => state.sunusantegatewayservice.userAccount.totalItems);
  ...
  return (
    <div>
      <h2 id="user-account-heading" data-cy="UserAccountHeading">
        <Translate contentKey="sunusanteGatewayServiceApp.userAccount.home.title">User Accounts</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="sunusanteGatewayServiceApp.userAccount.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/user-account/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="sunusanteGatewayServiceApp.userAccount.home.createLabel">Create new User Account</Translate>
          </Link>
        </div>
      </h2>
        ...
        {userAccountList && userAccountList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="sunusanteGatewayServiceApp.userAccount.id">ID</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('mfaSecret')}>
                  <Translate contentKey="sunusanteGatewayServiceApp.userAccount.mfaSecret">Mfa Secret</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('mfaSecret')} />
                </th>
            ...
            <tbody>
              {userAccountList.map((userAccount, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{userAccount.mfaSecret}</td>
                  <td>
                    <Translate contentKey={`sunusanteGatewayServiceApp.UserRole.${userAccount.role}`} />
                  </td>
                  ...
                  <td>
                    {userAccount.lastModifiedDate ? (
                      <TextFormat type="date" value={userAccount.lastModifiedDate} format={APP_DATE_FORMAT} />
                    ) : null}
                  </td>
                  <td>{userAccount.internalUser ? userAccount.internalUser.login : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="sunusanteGatewayServiceApp.userAccount.home.notFound">No User Accounts found</Translate>
            </div>
          )
        )}
      </div>
