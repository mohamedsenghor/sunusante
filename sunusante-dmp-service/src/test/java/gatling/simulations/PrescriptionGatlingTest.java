 ...
 */
public class PrescriptionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all prescriptions")
                    .get("/services/sunusantedmpservice/api/prescriptions")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new prescription")
                        .post("/services/sunusantedmpservice/api/prescriptions")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created prescription")
                            .get("/services/sunusantedmpservice${new_prescription_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created prescription")
                        .delete("/services/sunusantedmpservice${new_prescription_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Prescription entity").exec(scn);
