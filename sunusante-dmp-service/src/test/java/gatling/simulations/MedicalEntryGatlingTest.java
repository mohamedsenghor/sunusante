...
/**
 * Performance test for the MedicalEntry entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.6.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class MedicalEntryGatlingTest extends Simulation {

    String baseURL = Optional.ofNullable(System.getProperty("baseURL")).orElse("http://localhost:9042");
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all medicalEntries")
                    .get("/services/sunusantedmpservice/api/medical-entries")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new medicalEntry")
                        .post("/services/sunusantedmpservice/api/medical-entries")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_medicalEntry_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created medicalEntry")
                            .get("/services/sunusantedmpservice${new_medicalEntry_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created medicalEntry")
                        .delete("/services/sunusantedmpservice${new_medicalEntry_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MedicalEntry entity").exec(scn);
