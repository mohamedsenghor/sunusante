...
public class PatientGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all patients")
                    .get("/services/sunusantepatientservice/api/patients")
                ...
                .exec(
                    http("Create new patient")
                        .post("/services/sunusantepatientservice/api/patients")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created patient")
                            .get("/services/sunusantepatientservice${new_patient_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created patient")
                        .delete("/services/sunusantepatientservice${new_patient_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
